...
GROQ_TIMEOUT = 30
MAX_HISTORY = 6  # после 6 сообщений — сжимаем
...
async def do_search(query: str) -> str:
    """Поиск через compound-beta, результат обрезаем до 500 символов"""
    loop = asyncio.get_event_loop()
    response = await asyncio.wait_for(
        loop.run_in_executor(None, lambda: groq_request(
            model=SEARCH_MODEL,
            messages=[{"role": "user", "content": f"Кратко ответь на вопрос (максимум 3 предложения): {query}"}],
            max_tokens=200,
        )),
...
async def ask_groq(chat_id: int, user_text: str) -> str:
    ...
    summaries = user_data.get("summaries", [])  # список всех прошлых резюме

    # Строим системный промпт — добавляем ВСЕ накопленные резюме
    ...
    # Если история длинная — сжимаем и ДОБАВЛЯЕМ к списку резюме
    if len(history) >= MAX_HISTORY:
        logger.info("Summarizing history for chat %d", chat_id)
        new_summary = await summarize_history(history)
        summaries.append(new_summary)  # добавляем, не перезаписываем!
        history = []  # очищаем текущую историю

    user_data["history"] = history
    user_data["summaries"] = summaries
...
async def cmd_clear(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    save_user_data(chat_id, {"history": [], "summaries": []})
...
async def cmd_memory(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    text = f"🧠 Память:\n\nСообщений в текущей сессии: {history_len}\n"
    text += f"Сохранённых резюме: {len(summaries)}\n\n"
    if summaries:
