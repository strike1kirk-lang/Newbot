...
async def ask_groq(chat_id: int, user_text: str) -> str:
    ...
    response = await asyncio.wait_for(
        loop.run_in_executor(None, lambda: groq_request(
            model=os.environ.get("GROQ_MODEL", TEXT_MODEL),
...
async def ask_groq_vision(chat_id: int, image_url: str, caption: str) -> str:
    """Запрос к vision модели с фото"""
    loop = asyncio.get_event_loop()
    history = get_history(chat_id)
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    async with get_semaphore(chat_id):
        try:
            ...
            file_url = f"https://api.telegram.org/file/bot{os.environ['TELEGRAM_BOT_TOKEN']}/{file.file_path}"

            caption = update.message.caption or ""
            reply = await ask_groq_vision(chat_id, file_url, caption)
            await update.message.reply_text(reply)
        except asyncio.TimeoutError:
            await update.message.reply_text("⏱ Groq не ответил вовремя. Попробуй ещё раз.")
        except Exception as exc:
