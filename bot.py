...
# Темы при которых используем compound-beta
CURRENT_EVENTS_KEYWORDS = [
    "сегодня", "вчера", "сейчас", "новости", "произошло", "случилось",
    "погода", "курс", "цена", "вышел", "вышла", "анонс", "релиз",
    "today", "yesterday", "news", "weather", "price", "release", "just"
]
...
def needs_current_info(text: str) -> bool:
    """Проверяет нужна ли актуальная информация"""
    text_lower = text.lower()
    return any(kw in text_lower for kw in CURRENT_EVENTS_KEYWORDS)
...
async def ask_groq(chat_id: int, user_text: str) -> str:
    ...
    # Выбираем модель
    model = CURRENT_MODEL if needs_current_info(user_text) else TEXT_MODEL
    logger.info("Using model: %s for: %s", model, user_text[:50])
